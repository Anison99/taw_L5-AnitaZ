const posts = [
 ...
 const dataEndpoint = (router) => {
    ...
    // endpoint zwracający pojedynczy post o danym ID
    ...
    // endpoint modyfikujący pojedynczy post o danym ID
    router.put('/api/posts/:id', async (request, response, next) => {
        ...
        if (!post) {
            ...
            const newPost = request.body.updatedPost;
            post.title = newPost.title || post.title;
            post.text = newPost.text || post.text;
            post.image = newPost.image || post.image;
            response.status(200).send({post: post});
        }
    });

    // endpoint dodający nowy post
